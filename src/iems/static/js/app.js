...
$("#recovery-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; try { const payload = Object.fromEntries(new FormData(form)); const data = await api("/api/auth/forgot-password", { method: "POST", body: JSON.stringify(payload) }); closeModalCompat($("#recovery-modal")); form.reset(); toast(data.message); } catch (error) { toast(error.message, true); } });
...
async function signOut() { try { await api("/api/auth/logout", { method: "POST" }); location.reload(); } catch (error) { toast(error.message, true); } }
$("#logout-button").addEventListener("click", signOut);
$("#mobile-logout").addEventListener("click", signOut);
